        ...
        public async Task<IActionResult> ScrapeProduct([FromQuery] string url)
        {
            ...
            try
            {
                ...
                using (HttpClient client = new HttpClient())
                {
                    ...
                    var brandNode = document.DocumentNode.SelectSingleNode("//meta[@itemprop='brand']/@content//text()");
                    if (brandNode != null)
                    {
                        product.Brandofproduct = brandNode.InnerText.Trim();
                    }

                    var nameNode = document.DocumentNode.SelectSingleNode("//div[contains(@class,\"offer-selector__name--large\")]/h1//text()");
                    if (nameNode != null)
                    {
                        product.Name = nameNode.InnerText.Trim();
                    }

                    var modelNode = document.DocumentNode.SelectSingleNode("//*[@id=\"productModelValue\"]//content");
                    if (modelNode != null)
                    {
                        product.Model = modelNode.InnerText.Trim();
                    }

                    var mrp = document.DocumentNode.SelectSingleNode("//*[@id=\"wrapper\"]//div[contains(@class,\"offer-selector__add2cart-wrapper\")]/section[1]/div/div[2]//content");
                    if (mrp != null)
                    {
